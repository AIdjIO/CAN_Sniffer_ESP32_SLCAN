...
#define TWAI_DATA_ID            0x000                   

// Buffer for burst data handling
#define POLL_DEPTH              1024
...
void app_main(void)
{
    ...
    twai_mask_filter_config_t data_filter = {
        .id = TWAI_DATA_ID,
        .mask = 0x00,      // Match high 7 bits of the ID, ignore low 4 bits
        .is_ext = true,    // Receive only standard ID
    };
    ...
    while (1) {
        if (xSemaphoreTake(twai_listener_ctx.rx_result_semaphore, portMAX_DELAY) == pdTRUE) {
            ...
            /* **********************************************************  */
            /* *** write to console in SLCAN Protocol (i.e ASCII string) ***/
            /* *************** that SavvyCAN understands *************** ***/
            /* *********************************************************** */                    
            if (frame->header.ide == 1)
            {
                printf("T");
                printf("%08x%d", (unsigned int)(frame->header.id & 0xFFFFFFFF), frame->header.dlc);
            } else 
            {
                printf("t");
                printf("%03x%d", (unsigned int)(frame->header.id & 0xFFF), frame->header.dlc);
            }            
            
            for (int i = 0; i < frame->header.dlc; i++) {
                printf("%02x", frame->buffer[i]);
            }
            printf("\r"); // End of SLCAN frame
